	...
	t.prototype = {
		...
		run: function () {
			...
			(t.diff = ((t.loaded / 100) * Math.PI * 2 * 10).toFixed(2)),
				...
				t.context.beginPath(),
				t.context.arc(35, 35, 30, t.start, t.diff / 10 + t.start, !1),
				t.context.stroke(),
				t.loaded >= t.total && clearInterval(t.timer),
				t.loaded++;
		},
	};
	var e = function (t) {
		(this.bars = document.querySelectorAll(t)),
			this.bars.length > 0 && this.init();
	};
	(e.prototype = {
		init: function () {
			(this.tick = 25), this.progress();
		},
		progress: function () {
			var e = this,
				n = 0,
				i = e.bars[0].querySelector('canvas'),
				r =
					(new t(i),
					setInterval(function () {
						n++;
						var i = e.bars[n].querySelector('canvas');
						new t(i);
						n == e.bars.length && clearInterval(r);
					}, 100 * e.tick));
		},
	}),
		document.addEventListener('DOMContentLoaded', function () {
			new e('.single-pie-bar');
		});
})();
